...
- **Trustindex Google Reviews widget (2026-02):** loader.js is loaded ONCE globally in `/app/frontend/public/index.html`; the `GoogleReviews.jsx` component fetches the widget's `content.html` directly from the Trustindex CDN and hands it to `window.TrustindexWidget` to activate (loads preset CSS, formats stars, wires up slider). This bypasses the flaky repeated-<script>-injection behavior that stopped painting the widget after two consecutive client-side navigations. The component is keyed by pathname (`<GoogleReviewsInner key={pathname} />`) so React fully remounts it on every SPA route change.
